...
describe("basicMoves", () => {
  ...
  describe("moveXIfFacingHorizontal", () => {
    it("when facing south, return same value", () => {
      expect(
        moveXIfFacingHorizontal({ x: 1, y: 1, facingDirection: S }, 5),
      ).toBe(1);
    });
    it("when facing north, return same value", () => {
      expect(
        moveXIfFacingHorizontal({ x: 1, y: 1, facingDirection: N }, 5),
      ).toBe(1);
    });
    it("when facing east, increment X", () => {
      expect(
        moveXIfFacingHorizontal({ x: 1, y: 1, facingDirection: E }, 5),
      ).toBe(2);
    });
    it("when facing west, decrement X", () => {
      expect(
        moveXIfFacingHorizontal({ x: 1, y: 1, facingDirection: W }, 5),
      ).toBe(0);
    });
    it("when goes beyond max X throw error", () => {
      expect(() =>
        moveXIfFacingHorizontal({ x: 1, y: 1, facingDirection: E }, 1),
      ).toThrow(
    ...
    it("when goes beyond 0 throw error", () => {
      expect(() =>
        moveXIfFacingHorizontal({ x: 0, y: 1, facingDirection: W }, 1),
      ).toThrow(
  ...
  describe("moveYIfFacingVertical", () => {
    it("when facing East, return same value", () => {
      expect(moveYIfFacingVertical({ x: 1, y: 1, facingDirection: E }, 5)).toBe(
        1,
      );
    });
    it("when facing West, return same value", () => {
      expect(moveYIfFacingVertical({ x: 1, y: 1, facingDirection: W }, 5)).toBe(
        1,
      );
    });
    it("when facing south, increment Y", () => {
      expect(moveYIfFacingVertical({ x: 1, y: 1, facingDirection: S }, 5)).toBe(
        2,
      );
    });
    it("when facing north, decrement Y", () => {
      expect(moveYIfFacingVertical({ x: 1, y: 1, facingDirection: N }, 5)).toBe(
        0,
      );
    });
    it("when goes beyond max Y throw error", () => {
      expect(() =>
        moveYIfFacingVertical({ x: 1, y: 1, facingDirection: S }, 1),
      ).toThrow(
    ...
    it("when goes beyond 0 throw error", () => {
      expect(() =>
        moveYIfFacingVertical({ x: 1, y: 0, facingDirection: N }, 1),
      ).toThrow(
